...
class DistrictModelParser:
    ...
    def __init__(self, yaml_path):
        ...
        self.standards_map = {building["id"]: building["standards"] for building in self.raw_data["buildings"]}
        ...
        for building in self.raw_data["buildings"]:
            for apartment in building["apartments"]:
                for room in apartment["rooms"]:
                    ...
                    room_id = f"{apartment['id']}:{room['id']}"
        ...
        for building in self.raw_data["buildings"]:
            ...
            if "internal_connections" in building:
                for connection in building["internal_connections"]:
                    self._apply_internal_connection(connection, building_standards)

            if "external_connections" in building:
                for connection in building["external_connections"]:
                    self._apply_external_connection(connection, building_standards)
    ...
    def _apply_external_connection(self, connection, standards):
        ...
        ua_windows = sum(window["area"] * standards[window["thermal_code"]]["u_value"] for window in connection.get("windows", []))
